...
- **FIREBAT** (коробка): код, контент и `audio-guide.service` — `/opt/audio_guide`, uvicorn `127.0.0.1:8090`.
...
Ключ MapKit: [Кабинет разработчика](https://developer.tech.yandex.ru/) → подключить MapKit Mobile SDK. Локально скопируйте `app/lib/secrets.example.dart` в `app/lib/secrets.dart` (файл в git не попадает) и вставьте ключ. Без ключа карта всё равно рисуется (OSM). Либо так:
...
На FIREBAT OpenAI идёт через HideMe OpenVPN (split-tunnel только на `api.openai.com`, не полный шлюз). Env на хосте подтягивается `deploy/secrets/apply-local.sh`.
...
Готовый файл: страница [https://audio.solovyshka.com/app/](https://audio.solovyshka.com/app/). В приложении, если на сервере больший `versionCode`, появляется кнопка «Обновить». Первый раз ставится вручную; дальше — эта кнопка. Подпись одна и та же. Старый APK с Windows (debug-ключ) один раз удалить и поставить заново.
