...
#   sudo bash deploy/vpn/hideme-openai.sh down
#   sudo bash deploy/vpn/hideme-openai.sh status
set -euo pipefail
...
_pick_conf() {
  ...
  for c in \
    "${HIDEME_OVPN_CONF:-}" \
    /opt/secrets/vpn/netherlands-split.ovpn \
    /opt/secrets/audio_guide/vpn/netherlands-split.ovpn \
    /opt/secrets/food_checking/vpn/netherlands-split.ovpn
  do
    if [[ -n "$c" && -f "$c" ]]; then
      printf '%s\n' "$c"
      return 0
...
do_up() {
  ...
  for i in $(seq 1 "$MAX_WAIT"); do
    tun=$(ip -o link show | awk -F': ' '$2 ~ /^tun[0-9]+/ {gsub(/@.*$/,"",$2); print $2; exit}')
    if [[ -n "$tun" ]] && ip -4 addr show dev "$tun" 2>/dev/null | grep -q 'inet '; then
      break
    fi
    sleep 1
  done
  ...
  echo "hideme-openai: up tun=$tun gw=$gw"
}
